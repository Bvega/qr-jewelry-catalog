...
`index.html` and `item.html` share a semantic header, primary navigation, main landmark, and community-oriented footer. Both pages use the local Between Us mark, the same wordmark treatment, the same five navigation destinations, and the same visual tokens.
...
M06 owns permanent routes and QR/share migration beyond the preserved numeric route. M07 owns final content and photo replacement. Search, sorting, advanced filters, Collection routes, URL filter state, and timestamp-derived chronology remain deferred.
